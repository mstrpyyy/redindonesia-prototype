...
export const DropdownNavButton = ({ name, menu, level = 0 }: IDropdownNavButton) => {
  const [isOpen, setIsOpen] = useState(false)

  const toggleOpen = () => setIsOpen(prev => !prev)

  // Check if any first-level children have sub-menus
    ...
    <div className={`w-full ${level === 0 ? 'flex flex-col items-center' : ''}`}>
      ...
      <button
        ...
        className={`
          ...
          ${
            level === 0 ? 'font-bold ' 
            : level === 1 ? 'font-semibold justify-between  w-full' 
            : level === 2 ? 'font-medium justify-between w-full' 
      ...
      >
        <ul className={`flex flex-col gap-2 mt-2 ${level === 0 && !hasNestedMenus ? 'items-center text-center font-extralight' : ''}`}>
          {menu.map((item, index) => (
            <li key={index} className={level > 0 ? 'w-full' : ''}>
              {item.menu ? (
                // Recursive: Render nested dropdown
                <DropdownNavButton
                  name={item.name}
                  menu={item.menu}
                  isActive={false}
                ...
                <Link
                  ...
                  style={{ paddingLeft: level >= 1 ? `${level * 1}rem` : '0' }}
                >
                  {item.name}
                </Link>
              )}
